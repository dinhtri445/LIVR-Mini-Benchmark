        ...
        if labels is not None and input_ids is not None:
            ...
            for b in range(batch_size):
                ...
                if len(img_positions) == 0:
                    ...
                    fm = torch.zeros(seq_len, seq_len, dtype=torch.float32, device=device).masked_fill(~m, -10000.0)
                ...
                if stage == 1:
                    ...
                    m = torch.zeros(seq_len, seq_len, dtype=torch.float32, device=device).masked_fill(~m_bool, -10000.0)
